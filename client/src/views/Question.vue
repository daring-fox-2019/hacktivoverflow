        ...
        <div class="col-md-8">
          <div class="card mb-4">
              ...
              <div class="row">
                <div class="col-2">
                  <div class="d-flex flex-row">
                    <a class="fas fa-arrow-alt-circle-up fa-2x mr-3" @click="upvotes"></a>
                    <p
                      style="font-size: 30px"
                    >{{ question.upvotes.length - question.downvotes.length }}</p>
                    <a class="fas fa-arrow-alt-circle-down fa-2x ml-3" @click="downvotes"></a>
                  </div>
                </div>
                <div class="col-10">
                  <h2 class="card-title">{{ question.title }}</h2>

                  <p class="card-text text-black" v-html="question.description"></p>
                  <div v-if="question.questioner.email == userLogin">
                    <a
                      class="btn btn-danger ml-2 btn-sm float-right"
            ...
            <div class="card-footer text-muted">
              ...
              <span>{{question.questioner.username}}</span>
            </div>
          </div>
        </div>
        <div class="col-md-2"></div>
          ...
          <div class="col-md-8">
            <div class="card mb-4">
                ...
                <div class="row">
                  ...
                  <div class="col-md-4">
                    <div class="d-flex flex-row float-right">
                      <i
                        class="fas fa-arrow-alt-circle-up fa-2x mr-3"
                        @click="upvotesAnswer(answer._id)"
                      ></i>
                      <p
                        style="font-size: 30px"
                      >{{ answer.upvotes.length - answer.downvotes.length }}</p>
                      <i
                        class="fas fa-arrow-alt-circle-down fa-2x ml-3"
                        @click="downvotesAnswer(answer._id)"
                      ></i>
                    </div>
                  </div>
              ...
              <div class="card-footer text-muted">
                ...
                <span>{{answer.responder.username}}</span>
              </div>
            </div>
          </div>
          <div class="col-md-2"></div>
    ...
    fetchData() {
      backend
        ...
        .then(({ data }) => {
          console.log(data);
          this.question = data
          console.log("question vue",this.question);
        })
        .catch(err => {
          console.log(err)
        });
